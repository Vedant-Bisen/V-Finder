    ...
    if (res.ok) {
      ...
      const cacheLabel =
        data.cache_hits !== undefined
          ? ` · Cache ${data.cache_hits}/${data.cache_hits + data.cache_misses}`
          : "";
...
const allViews = [viewSearch, viewDb, viewIngest, viewSettings, viewOnboarding];
...
document.addEventListener("keydown", (e) => {
  ...
  if (isMeta && (e.key === "," || (e.key === "S" && isMeta && e.shiftKey))) {
    e.preventDefault();
    switchView("settings");
  }
  ...
  if (e.key === "Escape") {
    searchInput.value = "";
    resultsContainer.innerHTML =
      '<div class="placeholder-state"><p>Start typing to explore your multimodal memory.</p></div>';
    searchInput.blur();
  }
});
...
async function performSearch(query) {
  ...
  try {
    const body = {
      ...
      favorite_only: currentFavoriteFilter,
    };
    ...
    if (results.length === 0) {
      resultsContainer.innerHTML =
        '<div class="placeholder-state"><p>No relevant memory matches found.</p></div>';
      return;
    }
    ...
    resultsContainer.innerHTML = results
      .map((result) => {
        ...
        if (isImage && result.file_path) {
          ...
          const txtPreview = result.preview
            ? `...${result.preview.substring(0, 40)}...`
            : "";
...
async function loadSettings() {
  try {
    ...
    settingsApiKey.placeholder = data.api_key_set
      ? `Current: ${data.api_key_masked}`
...
async function saveSettings() {
  ...
  if (settingsDataDir.value.trim())
    body.data_dir = settingsDataDir.value.trim();
...
async function loadDbList() {
  dbTbody.innerHTML =
    '<tr><td colspan="4" style="text-align:center;">Loading database...</td></tr>';
  try {
    ...
    if (!data.results || data.results.length === 0) {
      dbTbody.innerHTML =
        '<tr><td colspan="4" style="text-align:center;">Your memory database is empty. Add memories first!</td></tr>';
      return;
    }

    dbTbody.innerHTML = data.results
      .map((item) => {
...
window.deleteDocument = async function (id) {
  if (!confirm("Are you sure you want to permanently forget this memory?"))
    return;
  try {
...
if (btnTestOnboardingKey) {
  btnTestOnboardingKey.addEventListener("click", async () => {
    ...
    if (!key) {
      onboardingApiStatus.innerHTML =
        '<span style="color: #ff4d4f;">Please enter an API key first.</span>';
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        onboardingApiStatus.innerHTML =
          '<span style="color: #34c759;">✓ API key is valid!</span>';
...
const btnOnboardingSetupFolder = document.getElementById(
  "btn-onboarding-setup-folder",
);
const onboardingFolderStatus = document.getElementById(
  "onboarding-folder-status",
);
...
if (localStorage.getItem("vfinder-theme") === "light") {
  document.body.classList.add("light-theme");
  const lightBtn = Array.from(themeBtns).find(
    (b) => b.dataset.theme === "light",
  );
  if (lightBtn) {
    themeBtns.forEach((b) => b.classList.remove("active"));
